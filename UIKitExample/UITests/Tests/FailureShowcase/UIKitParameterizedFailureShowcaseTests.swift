...
@Tag("UIKit")
final class UIKitParameterizedFailureShowcaseTests: UIKitExampleTestCase {
    @DisplayName("[UIKit][Expected failure] Intentional title mismatch: {id}")
    @Story("Parameterized assertion failures")
    @ParameterizedTest(
        name: "[{index}] UIKit intentional mismatch {id}",
        cases: [
            ExpectedTitleFailureTestData(id: "empty", expectedTitle: ""),
            ExpectedTitleFailureTestData(id: "other-platform", expectedTitle: "XCEasy SwiftUI Example"),
